...
note・ブログ・エッセイ・会話調の解説では、執筆前に `references/genre-notes.md` の essay 節を読む。クイックでもこのジャンル確認は省略しない。業務文書用の結論先行や見出しの作り方を、そのまま当てない。
このForkでnote記事一式を作る場合は `../note-writing/SKILL.md` とルートの `style-profile.md` を併用する。文体だけの依頼では、内容企画や付属出力を増やさず、日本語の推敲に限定する。
...
エッセイでは、中心となる場面や違和感を確認できればよい。見出しや結論を新設するために話を膨らませない。小見出しは場面や論点が切り替わり、読者が迷う箇所にだけ置く。説明を求められた記事は、その説明を保ったまま語り口を整える。
...
エッセイ・会話調の解説では `genre-notes.md` の差分を優先する。短文化、独立した一文、語尾の変化をノルマにせず、意味のまとまりで段落を作る。指定字数と太字の条件も、一般則よりユーザーの指定を優先する。
...
2. 各見出しが文書の用途に合うか（業務文書ではメッセージ、エッセイでは場面や論点の区切り。結論見出しへ一律に直さない）
